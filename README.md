...
- 当前版本：`12.2.0.1`，版本号统一维护在根目录 `Directory.Build.props` 的 `<Version>` 节点。
...
`SearchListBox` 和 `Transfer` 的数据源使用可变 `IList`，支持普通 `ObservableCollection<T>`；复杂数据项可通过 `ItemTemplate` 自定义显示。
