...
- 明确 Demo 保留 C# 预览语法，以及仓库不维护 `global.json`、由 CI 选择 .NET 11 SDK 的构建策略。

## 12.2.0.2 (2026-09-22)

- 修复主题转换器的单向绑定反向转换异常，未知状态值统一安全回退。
- 将状态颜色表改为只读暴露，图片转换器控制流资源并复用可缓存的位图实例。
- 移除依赖未定义 Git 标签的旧版程序集版本脚本。
...
- 修复 Windows 11 原生圆角模式错误清除 `CodeWFWindow` 外框描边的问题。
- 普通窗口继续使用主题配置的边框；最大化和全屏状态仍自动隐藏边框。
- 日志组件依赖升级到 12.1.0.23。

## 12.1.0.3 (2026-07-27)

- 修复 `CodeWFWindow` 多层相同圆角重复裁剪导致的窗体边缘锯齿和边框粗细不一致。
- 窗体背景、边框、圆角和裁剪统一由 `PART_WindowFrame` 负责，内容层改为透明结构容器。
- 最大化和全屏状态仅切换唯一窗框边界，避免内外层圆角状态不一致。
- Windows 11 使用 DWM 原生窗体圆角和非透明合成表面，避免透明顶级窗体四角描边断裂与锯齿。
- 原生圆角状态下取消 Avalonia 外框圆角和描边，窗体轮廓统一交由系统合成器绘制。
